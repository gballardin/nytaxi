...
* It is a big dataset from public data standards.  
    + The [flight delay dataset](http://www.transtats.bts.gov/DL_SelectFields.asp?Table_ID=236&DB_Short_Name=On-Time), normally referenced for out-of-memory calculations, is 100M+ record long.  
    + Engineering challenge to deal with such a large dataset.
* Structure:  
    + Trip dataset  
    + Fare dataset  
...
show how:
- total amount is broken down (interesting applications about taxes, tips). how to change peoples' decisions.
- geo-time dimensions
...
* **Include risk aversion in the calculations**
    + Do not just maximise profit, but the route that gives the optimal {profit, variance(profit)} tradeoff given the taxi driver preferences

## Potential applications  
As far as I can tell, [Uber](https://www.uber.com/features) does not disclose information about the passenger destination at ping time. Reasons:  

1. Avoid ride 'cherry-picking'.  
2. Cover the low end of the market (e.g. $5 ride).  

A counter-argument is that freer information leads to a better market allocation. Intuition:  

1. Decrease supply of unprofitable rides ($5 >> $8).  
2. Increase supply of profitable rides ($45 >> $40).  

Besides there is no point in asking a drive to take on negtive profit rides.

With the appropriate data, which is the case for NYC, my algorithm could help Uber and other drivers make the decision whether to accept a ride ex-ante, based on expected profit calculations.  

## Code  {.flexbox .vcenter}
* Code used to download, clean, analyse, visualize, and present this dataset can be found on [GitHub](https://github.com/gballardin/nytaxi).  
